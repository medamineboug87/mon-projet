...
Version corrigée avec gestion des NaN
"""
...
imputer = SimpleImputer(strategy='median')  # ← AJOUTÉ : remplace les NaN par la médiane

# ═══════════════════════════════════════════════════════════════
# MODÈLE 1 — FATIGUE (garde l'existant)
# ═══════════════════════════════════════════════════════════════
...
df_fatigue = df_fatigue.dropna(subset=[
    "Heart Rate", "Exercise Intensity", "Duration", "BMI", "Age"
])

df_fatigue["fatigue"] = (
    df_fatigue["Heart Rate"] > df_fatigue["Heart Rate"].quantile(0.75)
).astype(int)
...
# ═══════════════════════════════════════════════════════════════
# MODÈLE 2 — BLESSURE (CORRIGÉ AVEC IMPUTATION DES NaN)
# ═══════════════════════════════════════════════════════════════

print("📊 Chargement du dataset Multimodal Sports Injury...")
df_injury = pd.read_csv("d:/gym-ai/multimodal_sports_injury_dataset.csv")

print(f"✅ {len(df_injury)} lignes chargées")

# ═══════════════════════════════════════════════════════════════
# NETTOYAGE : Supprimer les NaN de la colonne cible d'abord
# ═══════════════════════════════════════════════════════════════
...
# 1. Encoder le genre
df_injury['gender_encoded'] = le.fit_transform(df_injury['gender'])  # 0=Femme, 1=Homme

# 2. Binariser la target
...
if len(unique_values) == 3:
    ...
    print("✅ Target convertie en binaire: 0=Healthy, 1=Injured/Risk")
else:
...
# Features d'entraînement (avec gestion des NaN)
...
# SMOTE (normalement plus d'erreur maintenant)
smote = SMOTE(random_state=42, sampling_strategy=1.0)
