...
public class AIService {
    ...
    public Map<String, Object> predictFatigueWithAI(Member member, List<TrainingSession> sessions) {
        try {
            ...
            TrainingSession lastSession = sessions.isEmpty() ? null : sessions.get(0);
            ...
            request.put("duration", lastSession != null ? lastSession.getDuration() : 60);
            request.put("weightLifted", getEffectiveWeight(lastSession));
            request.put("intensity", lastSession != null ? lastSession.getIntensity() : 5);
            request.put("hasCardio", lastSession != null && lastSession.getHasCardio() ? 1 : 0);
            request.put("cardioDurationMinutes", lastSession != null ? lastSession.getCardioDurationMinutes() : 0);
            request.put("cardioIntensity", lastSession != null ? lastSession.getCardioIntensity() : 0);
            request.put("muscleRiskScore", lastSession != null && lastSession.getExerciseBasedMuscleRiskScore() != null
                    ? lastSession.getExerciseBasedMuscleRiskScore() : 1.0);
            request.put("recoveryDaysPerWeek", lastSession != null && lastSession.getRecoveryDaysPerWeek() != null
            ...
            request.put("painLevel", lastSession != null && lastSession.getPainLevel() != null
                    ? lastSession.getPainLevel() : 0);
    ...
    public Map<String, Object> predictFatigueHeuristic(Member member, List<TrainingSession> sessions) {
        ...
        double fatigueScore = (sessionCount * 0.25) + (totalIntensity * 0.3) + (totalVolume / 1000 * 0.2)
                + (avgDuration / 60 * 0.15) + (painLevel / 10.0) * 0.1;
    ...
    public Map<String, Object> predictInjuryHeuristic(Member member, List<TrainingSession> sessions) {
        Map<String, Object> result = new LinkedHashMap<>();
        ...
        double injuryRisk = (fatigueScore / 10) * 0.25 + (painOccurrences / Math.max(1, sessions.size())) * 0.15
                + rapidProgressionRisk * 0.2 + imbalanceScore * 0.2 + recoveryScore * 0.1;
    ...
    public Map<String, Object> analyzeOverload(Member member, TrainingSession lastSession, List<TrainingSession> previousSessions) {
        ...
        if (Boolean.TRUE.equals(muscleRecovery.get("hasUnrecoveredMuscles"))) {
            ...
            if (recoveryWarnings != null) warnings.addAll(recoveryWarnings);
        }
        ...
        if (recommendedRestDays > 0) {
            recommendations.add("💤 Repos recommandé : " + recommendedRestDays + " jour(s).");
        }
    ...
    public Map<String, Object> calculateExerciseProgressions(Member member, TrainingSession lastSession) {
        ...
        for (SessionExercise currentEx : currentExercises) {
            ...
            Double allTimeMax = exerciseRepository.findMaxWeightByMemberAndExercise(member.getId(), exerciseName);
            ...
            if ("WARNING".equals(status)) {
                alerts.add(String.format("⚠️ %s : +%.0f%% de charge (seuil : +%.0f%%)", exerciseName, progressionPct, safeThreshold));
            } else if ("CRITICAL".equals(status)) {
                alerts.add(String.format("🔴 %s : +%.0f%% de charge en une séance ! Risque de blessure élevé.", exerciseName, progressionPct));
            }
        ...
        progressions.sort((a, b) -> {
            Map<String, Integer> order = Map.of("CRITICAL", 0, "WARNING", 1, "OK", 2, "DOWN", 3, "NEW", 4);
    ...
    private double calculateRecoveryDays(List<TrainingSession> sessions) {
        return sessions.stream().mapToInt(s -> s.getRecoveryDaysPerWeek() != null ? s.getRecoveryDaysPerWeek() : 2).average().orElse(2);
    }
    ...
    private double getAverageMuscleRiskScore(List<TrainingSession> sessions) {
        return sessions.stream()
                .mapToDouble(s -> s.getExerciseBasedMuscleRiskScore() != null ? s.getExerciseBasedMuscleRiskScore() : 1.0)
                .average().orElse(1.0);
    }
    ...
    private int getRecommendedRestDays(MemberProfile profile, TrainingSession session) {
        ...
        switch (level) {
            case "BEGINNER": restDays = 2; break;
            case "INTERMEDIATE": restDays = 1; break;
            case "ADVANCED": restDays = 1; break;
            case "ATHLETE": restDays = 0; break;
            default: restDays = 1;
        }
    ...
    private int getRecoveryHoursForMuscle(String muscleName) {
        return switch (muscleName.toLowerCase().trim()) {
            ...
            case "biceps", "biceps droit", "triceps", "triceps droit", "épaules", "trapèzes" -> 48;
            case "lombaires" -> 72;
            default -> 48;
        };
    }
    ...
    private List<Map<String, Object>> buildExerciseSummary(TrainingSession session) {
        ...
        for (SessionExercise ex : exercises) {
            ...
            if (ex.getWeightKg() != null && ex.getRepsCompleted() != null) {
                ...
                String chargeLevel = ex.getWeightKg() >= 100 ? "TRÈS ÉLEVÉE" : ex.getWeightKg() >= 70 ? "ÉLEVÉE" : ex.getWeightKg() >= 40 ? "MODÉRÉE" : "LÉGÈRE";
    ...
    private int parseReps(String repsCompleted) {
        ...
        try {
            if (repsCompleted.contains("-")) return Integer.parseInt(repsCompleted.split("-")[1]);
